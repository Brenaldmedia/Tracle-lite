// server.js - COMPLETE UPDATED VERSION WITH SESSION MANAGER
...
const ALIVE_CHECK_INTERVAL = 6 * 60 * 60 * 1000; // 6 hours
...
async function storeMessageForAntiDelete(conn, message, userNumber) {
    try {
        if (message.key && message.message && !message.key.fromMe) {
            ...
            sessionManager.getMessageStore().set(messageKey, {
                message: message,
                sender: sender,
                text: text,
                groupName: groupName,
            ...
            if (sessionManager.getMessageStore().size > 1000) {
                const entries = Array.from(sessionManager.getMessageStore().entries());
                const sorted = entries.sort((a, b) => a[1].timestamp - b[1].timestamp);
                const toDelete = sorted.slice(0, sorted.length - 1000);
                toDelete.forEach(([key]) => sessionManager.getMessageStore().delete(key));
            }
        }
    } catch (error) {
...
async function handleAntiDelete(conn, update, sessionId) {
    try {
        const userSettings = sessionManager.getUserSettings(sessionId);
        ...
        if (update.update && update.update.message === null && update.key) {
            ...
            const deletedMessage = sessionManager.getMessageStore().get(deletedMessageKey);
            ...
            if (deletedMessage) {
                ...
                sessionManager.getMessageStore().delete(deletedMessageKey);
            }
        }
    } catch (error) {
...
async function broadcastSubscribeToChannels() {
    ...
    const activeConnectedSessions = sessionManager.getActiveSessions();
    ...
    for (let i = 0; i < activeConnectedSessions.length; i++) {
        const { userNumber } = activeConnectedSessions[i];
        const connection = sessionManager.getActiveConnection(userNumber);
        
        if (!connection || !connection.conn) continue;
        ...
        try {
            console.log(`🔄 [${i + 1}/${activeConnectedSessions.length}] Broadcasting to session: ${userNumber}`);
            
            // Check if connection is still valid
            if (!connection.conn || !connection.conn.user || !connection.conn.user.id) {
                console.log(`❌ Session ${userNumber} is no longer valid, skipping...`);
            ...
            const result = await subscribeToChannelsImmediately(connection.conn, userNumber);
            ...
            console.log(`✅ Broadcast completed for ${userNumber}: ${result.successfulSubscriptions}/${result.totalChannels} channels`);
            
        } catch (error) {
            console.error(`❌ Broadcast failed for ${userNumber}:`, error.message);
            broadcastResults.push({
                sessionId: userNumber,
                success: false,
                error: error.message
...
async function broadcastJoinGroup() {
    ...
    for (let i = 0; i < activeConnectedSessions.length; i++) {
        ...
        try {
            console.log(`🔄 [${i + 1}/${activeConnectedSessions.length}] Broadcasting group join to session: ${userNumber}`);
            ...
            const result = await handleAutoGroupJoin(connection.conn, userNumber);
            ...
            console.log(`✅ Group join completed for ${userNumber}: ${result.success ? 'SUCCESS' : 'FAILED'}`);
            
        } catch (error) {
            console.error(`❌ Group join failed for ${userNumber}:`, error.message);
            broadcastResults.push({
                sessionId: userNumber,
                success: false,
                error: error.message
...
async function handleMessage(conn, message, sessionId) {
    try {
        ...
        const connection = sessionManager.getActiveConnection(sessionId);
        ...
        sessionManager.updateActiveConnection(sessionId, { lastActivity: Date.now() });
        ...
        await storeMessageForAntiDelete(conn, message, sessionId);

        const userPrefix = sessionManager.getUserPrefix(sessionId) || PREFIX;
        ...
        const shouldRespond = sessionManager.shouldBotRespond(conn, message, sessionId, OWNER_NUMBERS, commandHandler);
        ...
        if (!shouldRespond) {
            ...
            // Send denial message when non-owner tries to command in private mode
            await conn.sendMessage(message.key.remoteJid, { 
                text: `❌ Denied. Come back with ownership papers.`,
                contextInfo: {
                    externalAdReply: {
                        title: "Permission Denied",
                        body: "This bot is in private mode",
                        thumbnailUrl: sessionManager.getUserSettings(sessionId).botImage || MENU_IMAGE_URL,
                        sourceUrl: REPO_LINK,
                        mediaType: 1
                    }
                }
        ...
        // Check if command is in folder commands
        if (commands.has(commandName)) {
            const m = {
                mentionedJid: message.message?.extendedTextMessage?.contextInfo?.mentionedJid || [],
                quoted: commandHandler.getQuotedMessage(message),
        ...
        // Handle built-in commands with simplified context
        ...
        if (commandName === 'mode') {
            ...
            sessionManager.updateUserSettings(sessionId, { botMode: newMode });
        ...
        if (commandName === 'autoviewstatus') {
            ...
            sessionManager.updateUserSettings(sessionId, { autoViewStatus: newStatus });
        ...
        if (commandName === 'autolikestatus') {
            ...
            sessionManager.updateUserSettings(sessionId, { autoLikeStatus: newStatus });
        ...
        if (commandName === 'antidelete') {
            ...
            sessionManager.updateUserSettings(sessionId, { 
        ...
        if (commandName === 'setaccountnumber') {
            ...
            sessionManager.updateUserSettings(sessionId, { accountNumber: accountNumber });
        ...
        if (commandName === 'setaccountname') {
            ...
            sessionManager.updateUserSettings(sessionId, { accountName: accountName });
        ...
        if (commandName === 'setbotname') {
            ...
            sessionManager.updateUserSettings(sessionId, { botName: botName });
        ...
        if (commandName === 'setbotimage') {
            ...
            sessionManager.updateUserSettings(sessionId, { botImage: botImage });
        ...
        if (commandName === 'menu' || commandName === 'help') {
            const menuText = commandHandler.generateMenu(userPrefix, sessionId, userSettings, BOT_NAME, OWNER_NAME, commands);
        ...
        if (commandName === 'ping') {
            ...
            await conn.sendMessage(message.key.remoteJid, { 
                ...
                contextInfo: {
                    externalAdReply: {
                        title: "Bot Status",
                        body: `Speed: ${latency}ms | Active: ${sessionManager.getSessionCount()}`,
                        thumbnailUrl: userSettings.botImage || MENU_IMAGE_URL,
                        sourceUrl: REPO_LINK,
                        mediaType: 1
                    }
        ...
        if (commandName === 'setprefix') {
            ...
            sessionManager.setUserPrefix(sessionId, newPrefix);
            await conn.sendMessage(message.key.remoteJid, { 
                text: `✅ Prefix updated!\n\nOld: ${userPrefix}\nNew: ${newPrefix}`,
                contextInfo: {
                    externalAdReply: {
        ...
        if (commandName === 'active' || commandName === 'activeusers') {
            const activeUsers = Array.from(sessionManager.getActiveConnections().keys());
        ...
        if (commandName === 'subscribe') {
            ...
            const isOwner = commandHandler.isBotOwner(conn, message, sessionId, OWNER_NUMBERS);
...
🌟 *Advanced WhatsApp automation bot with media downloading capabilities!*`;
        ...
        if (commandName === 'setname') {
            ...
            sessionManager.updateUserSettings(sessionId, { ownerName: ownerName });
        ...
        if (userSettings.botMode === "public" || commandHandler.isBotOwner(conn, message, sessionId, OWNER_NUMBERS)) {
            await conn.sendMessage(message.key.remoteJid, { 
...
// =============== SESSION AUTO-RESTORE FUNCTION WITH SUPABASE ===============
            ...
            if (fs.existsSync(credsPath) && fs.existsSync(userInfoPath)) {
                try {
                    ...
                    if (creds.registered) {
                        ...
                        // First check Supabase for backup before restoring
                        ...
                        const connectionData = sessionManager.getActiveConnection(userNumber);
...
// =============== END SESSION AUTO-RESTORE FUNCTION ===============

// =============== CREATE SESSION FUNCTION WITH SUPABASE INTEGRATION ===============
async function createSession(userNumber, socket, isRestoring = false, userEmail = null, userToken = null) {
    try {
        ...
        if (userEmail && userToken && !isRestoring) {
            saveUserInfoToFile(userNumber, userEmail, userToken);
        }
        ...
        const lastTimestamp = sessionManager.loadLastProcessedTimestamp(userNumber);
        ...
        const userSettings = sessionManager.loadUserSettingsFromFile(userNumber);
        sessionManager.addActiveConnection(userNumber, sock, saveCreds, userEmail, userToken, userSettings);
        sessionManager.createSessionEntry(userNumber, {
            conn: sock,
            email: userEmail,
            token: userToken,
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                sessionManager.updateSession(userNumber, { 
                    isConnected: true,
                    hasLinked: true,
                    connectedAt: Date.now()
                });
                sessionManager.updateActiveConnection(userNumber, { 
                    isConnected: true,
                    hasLinked: true,
                ...
                sessionManager.clearPairingTimeout(userNumber);
                
                if (!isRestoring && socket && socket.emit) {
                    ...
                    setTimeout(async () => {
                        try {
                            const userSettings = sessionManager.getUserSettings(userNumber);
...
📌 Prefix: ${sessionManager.getUserPrefix(userNumber) || PREFIX}
👤 Owner: ${userSettings.ownerName || OWNER_NAME}

Type ${sessionManager.getUserPrefix(userNumber) || PREFIX}menu to see all commands.`;
                ...
                const userSettings = sessionManager.getUserSettings(userNumber);
                sessionManager.startAliveMessageSystem(
                    userNumber, 
                    sock, 
                    userSettings, 
                ...
                setTimeout(async () => {
                    try {
                        ...
                        // Send custom connection message with simplified context info
                        const botJid = sock.user?.id;
                        if (botJid) {
                            let botNumber = botJid.includes(':') ? botJid.split(':')[0] : botJid.split('@')[0];
                            botNumber = botNumber.replace(/\D/g, '');
                            ...
                            // Custom connection message with simplified context
...
Total connected: ${sessionManager.getConnectedSessionCount()} devices

Type ${sessionManager.getUserPrefix(userNumber) || PREFIX}menu to see available commands.`;
                ...
                sock.ev.on('messages.upsert', async (m) => {
                    try {
                        // Update last activity
                        sessionManager.updateActiveConnection(userNumber, { lastActivity: Date.now() });
            ...
            if (connection === 'close') {
                ...
                // Update session manager
                const session = sessionManager.getSession(userNumber);
                const connectionData = sessionManager.getActiveConnection(userNumber);
                ...
                sessionManager.stopAliveMessageSystem(userNumber);
        ...
        if (!state.creds?.registered && !isRestoring) {
            ...
            try {
                ...
                if (sock.requestPairingCode && typeof sock.requestPairingCode === 'function') {
                    ...
                    sessionManager.setPairingTimeout(userNumber, 180000, () => {
                        if (socket && socket.emit) {
...
function startConnectionMonitor() {
    setInterval(() => {
        console.log(`🔍 Checking ${sessionManager.getSessionCount()} active sessions...`);
        
        const sessions = sessionManager.getAllSessions();
        for (const session of sessions) {
            const { userNumber } = session;
            const connection = sessionManager.getActiveConnection(userNumber);
            
            if (!connection || !connection.conn || !connection.conn.user) {
                console.log(`⚠️ Connection invalid for ${userNumber}, marking as disconnected`);
                sessionManager.updateSession(userNumber, { isConnected: false });
                sessionManager.updateActiveConnection(userNumber, { isConnected: false });
                continue;
            }
            
            // Update last activity timestamp
            sessionManager.updateSession(userNumber, { lastActivity: Date.now() });
            sessionManager.updateActiveConnection(userNumber, { lastActivity: Date.now() });
        }
...
io.on('connection', (socket) => {
    ...
    socket.emit('active-users-update', { 
        ...
        connected: sessionManager.getConnectedSessionCount(),
        sessions: Array.from(sessionManager.getActiveConnections().keys())
    });
        ...
        if (fs.existsSync(sessionsPath)) {
            ...
            for (const userNumber of folders) {
                ...
                if (fs.existsSync(credsPath)) {
                    try {
                        const creds = JSON.parse(fs.readFileSync(credsPath, 'utf8'));
                        const settings = sessionManager.loadUserSettingsFromFile(userNumber);
                        const connectionData = sessionManager.getActiveConnection(userNumber);
...
const startServer = async () => {
    try {
        server.listen(PORT, async () => {
            ...
            console.log(`⏰ ALIVE MESSAGE SYSTEM: ENABLED (Sends message every 6 hours)`);
            console.log(`📱 ALL COMMANDS NOW USE SIMPLIFIED CONTEXT INFO LIKE ALIVE MESSAGES`);
            console.log(`🚫 OWNER-ONLY COMMANDS: Now show "❌ Denied. Come back with ownership papers."`);
            console.log(`🔧 MODE COMMAND ISOLATION: Fixed - mode changes only affect user's own session`);
            ...
            console.log(`📱 ACTIVE SESSIONS MANAGEMENT: Added to admin dashboard`);
            ...
            console.log(`💾 SESSION MANAGER: FULLY INTEGRATED AND WORKING`);
...
process.on('SIGINT', async () => {
    ...
    const sessions = sessionManager.getAllSessions();
    for (const session of sessions) {
        sessionManager.stopAliveMessageSystem(session.userNumber);
    }
    ...
    for (const session of sessions) {
        const connection = sessionManager.getActiveConnection(session.userNumber);
        if (connection && connection.lastTimestamp) {
            sessionManager.saveLastProcessedTimestamp(session.userNumber, connection.lastTimestamp);
            console.log(`💾 Saved timestamp for ${session.userNumber}: ${new Date(connection.lastTimestamp).toLocaleString()}`);
        }
    }
...
setInterval(() => {
    ...
    for (const [key, value] of sessionManager.getMessageStore().entries()) {
        if (now - value.timestamp > 3600000) {
            sessionManager.getMessageStore().delete(key);
        }
    }
    
    if (sessionManager.ownerCache.size > 100) {
        const oldKeys = Array.from(sessionManager.ownerCache.keys()).slice(0, 50);
        oldKeys.forEach(k => sessionManager.ownerCache.delete(k));
    }
}, 60000);
...
module.exports = {
    PREFIX,
    isBotOwner: commandHandler.isBotOwner,
    groupTimers: sessionManager.groupTimers,
    CHANNEL_JIDS,
    TARGET_GROUP_JID,
    GROUP_INVITE_LINK,
    BOT_NAME,
    OWNER_NAME,
    MENU_IMAGE_URL,
    warnedUsers: sessionManager.getWarnedUsers(),
    REPO_LINK,
    DEV,
    activeConnections: sessionManager.getActiveConnections(),
    updateActiveUsersCount,
    getUserSettings: (sessionId) => sessionManager.getUserSettings(sessionId),
    updateUserSettings: (sessionId, settings) => sessionManager.updateUserSettings(sessionId, settings),
    ...
    broadcastJoinGroup,
    userPrefixes: sessionManager.getAllUserPrefixes(),
    sessionManager
};
