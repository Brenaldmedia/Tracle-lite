// SERVER.JS (COMPLETE UPDATED VERSION WITH SEPARATED COMMANDS)
...
// =============== UPDATED: ALIVE MESSAGE SYSTEM WITH SIMPLIFIED CONTEXT INFO & 6 HOURS ===============
const ALIVE_CHECK_INTERVAL = 6 * 60 * 60 * 1000; // CHANGED: 6 hours instead of 2 hours
...
Anyway… stay chaotic. 🌚`;
            ...
            // UPDATED: Use simplified context info like alive messages
...
function shouldBotRespond(conn, message, sessionId) {
    try {
        ...
        // FIXED: Check if this user is the owner of this specific session
        ...
        if (userSettings.botMode === "public") {
            ...
            return true;
        } else if (userSettings.botMode === "private" && isOwner) {
            console.log(`✅ Private mode - owner can command`);
            return true;
        }
...
async function handleMessage(conn, message, sessionId) {
    try {
        ...
        if (!shouldRespond) {
            ...
            // FIXED: Send denial message when non-owner tries to command in private mode
            await conn.sendMessage(message.key.remoteJid, { 
        ...
        // MODE COMMAND - FIXED: User isolated mode
        if (commandName === 'mode') {
...
Type ${userPrefixes.get(userNumber) || PREFIX}menu to see all commands.`;
                ...
                // =============== START ALIVE MESSAGE SYSTEM (6 HOURS) ===============
