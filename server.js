...
const PTERODACTYL_URL = 'https://burzor.prexzyvilla.site';
const PTERODACTYL_API_KEY = process.env.PTERODACTYL_API_KEY || '';
const PTERODACTYL_SERVER_ID = 'fbe19ccc';

// Check if we should proxy to Pterodactyl
const SHOULD_PROXY_TO_PTERODACTYL = IS_HEROKU && PTERODACTYL_API_KEY;
const IS_PTERODACTYL_PRIMARY = IS_PTERODACTYL || !IS_HEROKU;

console.log(`🔗 Pterodactyl Proxy: ${SHOULD_PROXY_TO_PTERODACTYL ? 'ENABLED' : 'DISABLED'}`);
console.log(`🏭 Primary Bot Processor: ${IS_PTERODACTYL_PRIMARY ? 'Pterodactyl' : 'Heroku'}`);
...
// =============== PROXY MIDDLEWARE FOR HEROKU ===============
const proxyToPterodactyl = async (req, res) => {
    if (!SHOULD_PROXY_TO_PTERODACTYL || !IS_HEROKU) {
        return false;
    }

    try {
        console.log(`🔄 Heroku proxying to Pterodactyl: ${req.method} ${req.path}`);
        
        const pterodactylEndpoint = `${PTERODACTYL_URL}/server/${PTERODACTYL_SERVER_ID}`;
        const targetUrl = `${pterodactylEndpoint}${req.path}`;
        ...
        if (error.response) {
            // Pterodactyl responded with error
            res.status(error.response.status).json({
                error: 'Pterodactyl proxy error',
                message: error.response.data?.message || error.message,
                status: error.response.status
            });
        } else if (error.request) {
            // No response from Pterodactyl
            res.status(502).json({
                error: 'Pterodactyl unreachable',
                message: 'The bot processor is currently unavailable',
        ...
        return true; // We handled the request (even if with error)
    }
};

// =============== HEROKU-ONLY API ROUTES (Lightweight) ===============
if (IS_HEROKU) {
    console.log('🔄 Setting up Heroku proxy routes...');
    
    // Health check endpoint (Heroku stays awake with this)
    app.get('/api/health', (req, res) => {
        res.json({
            status: 'healthy',
            service: 'Tracle-Lite Heroku Proxy',
            mode: 'proxy',
            target: SHOULD_PROXY_TO_PTERODACTYL ? 'Pterodactyl' : 'Standalone',
    ...
    app.get('/api/status', async (req, res) => {
        try {
            if (SHOULD_PROXY_TO_PTERODACTYL) {
                // Try to get status from Pterodactyl
                const response = await axios.get(
                    `${PTERODACTYL_URL}/server/${PTERODACTYL_SERVER_ID}/api/status`,
                    {
                        headers: {
                            'Authorization': `Bearer ${PTERODACTYL_API_KEY}`
                        },
                ...
                res.json({
                    status: 'online',
                    mode: 'standalone',
                    message: 'Running in standalone mode on Heroku',
                    timestamp: new Date().toISOString()
                });
            }
        } catch (error) {
            res.json({
                status: 'proxy_error',
                mode: 'heroku_proxy',
                message: 'Unable to reach Pterodactyl bot processor',
                error: error.message,
                timestamp: new Date().toISOString()
            });
        }
    ...
    // Wake-up endpoint for Heroku (prevents sleep)
    app.get('/api/wake', (req, res) => {
        res.json({
            awake: true,
            timestamp: new Date().toISOString(),
            message: 'Heroku instance is awake'
        });
    });
    ...
    app.get('/api/test-pterodactyl', async (req, res) => {
        try {
            ...
            res.status(500).json({
                success: false,
                message: 'Failed to connect to Pterodactyl',
                error: error.message,
                serverId: PTERODACTYL_SERVER_ID
            });
        }
...
app.use('/api', async (req, res, next) => {
    ...
    if (req.path === '/health' || req.path === '/status' || req.path === '/wake' || req.path === '/test-pterodactyl') {
        return next();
    }
    
    if (IS_HEROKU && SHOULD_PROXY_TO_PTERODACTYL) {
        const wasProxied = await proxyToPterodactyl(req, res);
...
if (IS_HEROKU && !IS_PTERODACTYL) {
    ...
    server.listen(BACKEND_PORT, () => {
        ...
        ✅ Frontend connects directly to Pterodactyl
        ============================================
        `);
    });
    
    module.exports = { app, server };
    return; // STOP HERE - Don't load bot logic on Heroku
}
// =============== SKIP ON HEROKU KEEP-ALIVE ===============
if (IS_HEROKU && !IS_PTERODACTYL) {
    // Already handled above - should not reach here
    console.log('⚠️ This should not be reached on Heroku keep-alive mode');
    return;
}
// =============== CREATE HTTP SERVER ===============
const server = http.createServer(app);
...
// =============== SOCKET.IO PROXY FOR HEROKU ===============
if (IS_HEROKU && SHOULD_PROXY_TO_PTERODACTYL) {
    console.log('🔌 Setting up Socket.IO proxy to Pterodactyl...');
    
    // Connect to Pterodactyl's Socket.IO server
    let pterodactylSocket = null;
    ...
    const connectToPterodactylSocket = () => {
        try {
            ...
            // For now, we'll handle it via API calls
            
        } catch (error) {
            console.error('❌ Failed to connect to Pterodactyl Socket.IO:', error.message);
        }
    };
    
    // Initial connection attempt
    connectToPterodactylSocket();
}
...
server.listen(BACKEND_PORT, () => {
    console.log(`
    ============================================
    🚀 TRACLE-LITE V2 - HYBRID DEPLOYMENT
    ============================================
    ...
    🔄 Proxy Mode: ${IS_HEROKU && SHOULD_PROXY_TO_PTERODACTYL ? 'ENABLED' : 'DISABLED'}
    🏭 Processor: ${IS_PTERODACTYL_PRIMARY ? 'Pterodactyl' : 'Heroku'}
    ============================================
...
// =============== PTERODACTYL-ONLY BOT LOGIC ===============
if (IS_PTERODACTYL_PRIMARY) {
    console.log('🤖 Loading Pterodactyl bot processor...');
    
    // Import all bot modules only on Pterodactyl
    ...
    app.get('/api/connect-test', (req, res) => {
        res.json({
            connected: true,
            server: 'Tracle-Lite Bot Processor',
            port: BACKEND_PORT,
            socketIO: true,
            cors: 'configured',
    ...
    app.get('/api/test-connection', (req, res) => {
        res.json({
            success: true,
            message: 'Pterodactyl bot processor is running!',
            timestamp: new Date().toISOString(),
    ...
    app.get('/api/heroku-test', (req, res) => {
        res.json({
            success: true,
            message: 'Pterodactyl bot processor accessible from Heroku Frontend',
            frontendOrigin: req.headers.origin,
            timestamp: new Date().toISOString(),
...
Anyway… stay chaotic. 🌚`;
    ...
    app.get('/api/health', (req, res) => {
        ...
        res.json({
            status: 'healthy',
            service: 'Tracle-Lite V2 Bot Processor',
            version: '2.0.0',
            botName: BOT_NAME,
            ownerName: OWNER_NAME,
            ...
            timestamp: new Date().toISOString(),
            processor: 'pterodactyl',
            supports: ['Heroku Proxy', 'WebSocket', 'Token-Free System']
        });
    });
    ...
    const startBotProcessor = async () => {
        try {
            console.log(`\n🤖 ${BOT_NAME} Bot Processor running on port ${BACKEND_PORT}`);
            console.log(`👑 Owner: ${OWNER_NAME}`);
            console.log(`🏭 Processor: Pterodactyl Panel`);
            console.log(`🔗 Connected to: ${PTERODACTYL_URL}`);
            console.log(`🎯 Server ID: ${PTERODACTYL_SERVER_ID}`);
            ...
            console.log(`🔗 Bot Processor URL: https://node.burzor.prexzyvilla.site:${BACKEND_PORT}`);
            console.log(`🔌 Proxy from Heroku: ${IS_HEROKU ? 'YES' : 'NO'}`);
        ...
        if (fs.existsSync(indexPath)) {
            ...
            // Send status to Heroku (if configured)
            if (SHOULD_PROXY_TO_PTERODACTYL) {
                console.log(`🔗 Ready to receive requests from Heroku proxy`);
            }
    ...
    process.on('SIGINT', async () => {
        console.log('\n🔻 Shutting down Pterodactyl bot processor gracefully...');
    ...
    console.log('🔄 Heroku running in proxy mode - bot logic runs on Pterodactyl');
    
    // Heroku-specific exports (lightweight)
    module.exports = {
        app,
        server,
        io,
