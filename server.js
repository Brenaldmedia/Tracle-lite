...
server.listen(BACKEND_PORT, () => {
    console.log(`
    ============================================
    🚀 TRACLE-LITE V2 - HEROKU DEPLOYMENT
    ============================================
    📍 Environment: Heroku
...
// =============== HEROKU BOT LOGIC ===============
console.log('🤖 Loading Heroku bot processor...');
...
app.get('/api/health', (req, res) => {
    res.json({
        status: 'healthy',
        service: 'Tracle-Lite Heroku Bot Processor',
        mode: 'full',
        timestamp: new Date().toISOString(),
        uptime: process.uptime(),
        activeSessions: activeConnections ? activeConnections.size : 0
    });
});

app.get('/api/status', (req, res) => {
    const connectedSessions = activeConnections ? 
        Array.from(activeConnections.values()).filter(data => data.isConnected).length : 0;
    
    res.json({
        status: 'online',
        mode: 'heroku_full',
        message: 'Running in full mode on Heroku',
        activeSessions: activeConnections ? activeConnections.size : 0,
...
app.get('/api/heroku-test', (req, res) => {
    res.json({
        success: true,
        message: 'Heroku bot processor is running!',
        frontendOrigin: req.headers.origin,
        timestamp: new Date().toISOString(),
        environment: process.env.NODE_ENV || 'production',
        serverTime: new Date().toLocaleString(),
        herokuApp: process.env.HEROKU_APP_NAME || 'tracle-lite',
        version: '2.0.0',
        botName: BOT_NAME || 'Tracle-Lite'
    });
});
...
async function createSession(userNumber, socket, isRestoring = false, userEmail = null) {
    try {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        if (fs.existsSync(credsPath)) {
                            ...
                            if (creds.registered) {
                                console.log(`📱 Session ${userNumber} is registered, backing up...`);
                                
                                const backupResult = await backupManager.backupSessionToDrive(userNumber);
                                
                                if (backupResult.success) {
                                    console.log(`✅ Session ${userNumber} backed up successfully (${backupResult.backedUpFiles} files)`);
        ...
        sock.ev.on('creds.update', async (creds) => {
            ...
            if (creds.registered) {
                console.log(`💾 Credentials updated and registered for ${userNumber}, backing up...`);
                
                setTimeout(async () => {
                    try {
                        const backupResult = await backupManager.backupNewUserSession(userNumber);
                        if (backupResult.success) {
                            console.log(`✅ Credentials backed up for ${userNumber}`);
                        }
                    } catch (error) {
...
async function restoreExistingSessions() {
    ...
    try {
        // Check Supabase backup for sessions
        console.log('\n☁️ STEP 1: Checking backup for sessions...');
        
        if (backupManager.isConfigured()) {
            console.log('✅ Backup is configured, restoring from cloud backup...');
            
            console.log('\n📥 Downloading all data from backup...');
            ...
            if (dataRestoreResult.success) {
                ...
                console.log('📭 No data found in backup or restore failed');
            }
            
            await delay(3000);
        } else {
            console.log('⚠️ Backup not configured, skipping cloud restore');
        }
        
        // Now check if we have any sessions locally
...
app.post('/api/test-backup', async (req, res) => {
    try {
        ...
        // Check if session exists on backup
        ...
        res.json({
            ...
            existsOnBackup: checkResult.sessionExists,
...
app.get('/api/health', (req, res) => {
    ...
    res.json({
        ...
        backend: `Heroku:${BACKEND_PORT}`,
        timestamp: new Date().toISOString(),
        processor: 'heroku',
        supports: ['Full Bot Mode', 'WebSocket', 'Token-Free System']
    });
});
...
const startBotProcessor = async () => {
    try {
        console.log(`\n🤖 ${BOT_NAME} Bot Processor running on Heroku`);
        console.log(`👑 Owner: ${OWNER_NAME}`);
        console.log(`🏭 Processor: Heroku Full Mode`);
        ...
        console.log(`🔗 Bot Processor URL: https://your-app-name.herokuapp.com:${BACKEND_PORT}`);
        ...
        console.log(`✅ Heroku bot processor initialized.`);
...
process.on('SIGINT', async () => {
    console.log('\n🔻 Shutting down Heroku bot processor gracefully...');
