                ...
                const reply = (text, options = {}) => {
                    const contextOptions = {
                        ...
                        contextInfo: {
                            ...
                            externalAdReply: options.externalAdReply || { title: `${userSettings.botName || BOT_NAME} Command`, body: `Executed: ${commandName}`, thumbnailUrl: userSettings.botImage || MENU_IMAGE_URL, sourceUrl: REPO_LINK, mediaType: 1 }
                        }
                    };
                ...
                sock.ev.on('messages.upsert', async (m) => {
                    try {
                        ...
                        for (const msg of m.messages) {
                            ...
                            // === AUTO TYPING (only for groups or if enabled for all) ===
                            if (userSettings.autoTyping === "true") {
                                await sock.sendPresenceUpdate('composing', jid).catch(() => {});
                                await delay(800);
                            }

                            // === AUTO RECORDING (only for groups or if enabled for all) ===
                            if (userSettings.autoRecording === "true") {
                                await sock.sendPresenceUpdate('recording', jid).catch(() => {});
                                await delay(1200);
                            }

                            // Process message (commands, etc.)
