...
module.exports = {
    ...
    alias: ["fifaresults"],
    category: "sports",
    description: "Get FIFA match results",

    execute: async (conn, mek, m, { from, reply }) => {
        try {
            await conn.sendMessage(from, { react: { text: "📋", key: mek.key } });
            
            const response = await axios.get(`https://apiskeith.top/fifa/matches`, { timeout: 15000 });
            
            if (!response.data?.status) {
                return reply(`❌ Could not fetch FIFA matches.\n> ⚡ Powered by TRACLE-LITE`);
            }

            const matches = response.data.result || [];
            let message = `📋 *FIFA MATCH RESULTS* 📋\n━━━━━━━━━━━━━━━━━━━━\n\n`;
