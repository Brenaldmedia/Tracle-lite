...
module.exports = {
    pattern: "uclscorers",
    alias: ["championsscorers", "uclgoals"],
    category: "sports",
    description: "Get UCL top scorers",

    execute: async (conn, mek, m, { from, reply }) => {
        try {
            await conn.sendMessage(from, { react: { text: "⚽", key: mek.key } });
            
            const response = await axios.get(`https://apiskeith.top/ucl/scorers`, { timeout: 15000 });
            
            if (!response.data?.status) {
                return reply(`❌ Could not fetch UCL scorers.\n> ⚡ Powered by TRACLE-LITE`);
            }

            const scorers = response.data.result?.topScorers || [];
            let message = `⚽ *UCL TOP SCORERS* ⚽\n━━━━━━━━━━━━━━━━━━━━\n\n`;
