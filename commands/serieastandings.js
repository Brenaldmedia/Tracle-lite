...
module.exports = {
    pattern: "serieascorers",
    alias: ["serieagoals"],
    category: "sports",
    description: "Get Serie A top scorers",

    execute: async (conn, mek, m, { from, reply }) => {
        try {
            await conn.sendMessage(from, { react: { text: "⚽", key: mek.key } });
            
            const response = await axios.get(`https://apiskeith.top/seriea/scorers`, { timeout: 15000 });
            
            if (!response.data?.status) {
                return reply(`❌ Could not fetch Serie A scorers.\n> ⚡ Powered by TRACLE-LITE`);
            }

            const scorers = response.data.result?.topScorers || [];
            let message = `⚽ *SERIE A TOP SCORERS* ⚽\n━━━━━━━━━━━━━━━━━━━━\n\n`;
